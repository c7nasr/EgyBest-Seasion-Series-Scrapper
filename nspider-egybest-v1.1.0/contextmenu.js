function get_current_tab_url() {
  ...
  async function go_to_video_stream(api_links, current_domain) {
    return new Promise(async (resolve, reject) => {
      ...
      const cookie = await get_cookies("https://teco.egybest.site", "PSSID");
      const vsl = [];
      console.log(domain);
      for (let index = 0; index < api_links.length; index++) {
        try {
          ...
          myHeaders.append("Cookie", "PSSID=" + 'VuM2aVlDJR4sQPtJK%2Ca45SoO1BUquFN2N9O9zPs8VCnZHsxMSnYqymvCp9C80tE0AanuZ9mVGlI8LGcKn4THdClYcrb%2CQhYatE5phuREJ8baoCpde-zhnKf20yL8L2Vr');
  ...
  chrome.contextMenus.create({
    ...
    contexts: ["all"],
    onclick: start,
    documentUrlPatterns: ["*://*.egybest.site/season/*"],
  });